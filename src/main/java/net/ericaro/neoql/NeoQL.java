...
public class NeoQL {
	...
	public static <T> TableDef<T> select(TableDef<T> table){
		return new SelectTableDef<T>(new Select<T>(table, NeoQL.True));
	}
	...
	public static <S,T> TableDef<T> select(Mapper<S,T> mapper, Class<S> table){
		return new MapSelectTableDef<S,T>(new MapSelect<S,T>(mapper, table, NeoQL.True));
	}
	...
	public static <S,T> TableDef<T> select(Mapper<S,T> mapper, Class<S> table, Predicate<? super S> where){
		return new MapSelectTableDef<S,T>(new MapSelect<S,T>(mapper, table, where));
	}
